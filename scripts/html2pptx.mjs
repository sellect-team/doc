...
function extract() {
  ...
  document.querySelectorAll("section.slide").forEach((slide) => {
    ...
    const collect = (el) => {
      ...
      if (tag === "svg") {
        images.push({ kind: "svg", mark: el.dataset.pptxMark, ...rel(r) });
...
console.log(`  ${slides.length}장 · 도형 ${nShape}개 · 텍스트 줄 ${nText}개 · 그림 ${nImg}개(아이콘 ${icons.size})`);
